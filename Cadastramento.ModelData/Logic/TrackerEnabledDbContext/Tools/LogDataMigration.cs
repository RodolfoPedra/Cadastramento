...
namespace Cadastramento.ModelData.Logic.TrackerEnabledDbContext.Tools
{
    public class LogDataMigration
    {
        ...
        {
            return _trackerContext.Set<auditlogdetail>().Where(x => x.PropertyName == columnName
                            && x.auditlog.TypeFullName == typeFullname);
        }
        ...
        {
            return _trackerContext.Set<auditlog>().Where(x => x.TypeFullName == tableName);
        }
